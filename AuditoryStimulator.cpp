...
void AuditoryStimulator::setup()
{
  ...
  // Properties
  modular_server_.property(audio_controller::constants::polarity_reversed_property_name).setDefaultValue(constants::polarity_reversed_default);

  modular_server_.property(audio_controller::constants::channels_enabled_property_name).setDefaultValue(constants::channels_enabled_default);
  ...
  flipper_delay_property.setUnits(audio_controller::constants::ms_unit);
  ...
  flipper_period_property.setUnits(audio_controller::constants::ms_unit);
  ...
  flipper_on_duration_property.setUnits(audio_controller::constants::ms_unit);
  ...
  flip_enabled_function.attachFunctor(makeFunctor((Functor0 *)0,*this,&AuditoryStimulator::flipEnabledHandler));
  ...
  flip_callback.attachFunctor(makeFunctor((Functor0 *)0,*this,&AuditoryStimulator::flipHandler));
  ...
  flip_callback.attachTo(audio_controller::constants::switch_0_interrupt_name,modular_server::interrupt::mode_falling);
#endif

  // modular_server::Callback & enable_flip_callback = modular_server_.createCallback(constants::enable_flip_callback_name);
  // enable_flip_callback.attachFunctor(makeFunctor((Functor0 *)0,*this,&AuditoryStimulator::enableFlipHandler));

  // modular_server::Callback & disable_flip_callback = modular_server_.createCallback(constants::disable_flip_callback_name);
  // disable_flip_callback.attachFunctor(makeFunctor((Functor0 *)0,*this,&AuditoryStimulator::disableFlipHandler));
...
void AuditoryStimulator::flip(const ConstantString * const polarity_ptr)
{
...
void AuditoryStimulator::flipEnabledHandler()
{
  modular_server_.response().returnResult(flip_enabled_);
}

void AuditoryStimulator::enableFlipHandler()
{
  if (!flip_enabled_)
  {
    noInterrupts();
...
void AuditoryStimulator::disableFlipHandler()
{
  if (flip_enabled_)
  {
    noInterrupts();
...
void AuditoryStimulator::flipHandler()
{
  if (!flipping_ && flip_enabled_)
  {
